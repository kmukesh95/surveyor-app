...
async function seedMasterStatesClean() {
  console.log('🏛️ Inserting 36 Master States with clean two-digit codes ("01", "02", ...) without prefix...');
  ...
  for (const s of stateList) {
    ...
    console.log(`  ✓ ID ${s.id.toString().padStart(2, '0')}: Code "${s.stateCode}" -> ${s.stateName}`);
  }
  ...
  console.log('\n🎉 ALL 36 MASTER STATES INSERTED FRESH WITH NO PREFIX ("01", "02", ...)!');
}

seedMasterStatesClean()
  .catch((e) => {
    console.error('❌ Error seeding master states:', e);
    process.exit(1);
  })
  .finally(async () => {
